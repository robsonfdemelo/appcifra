...
exports.SUPPORTED_CHORD_SUFFIXES = void 0;
exports.parseChordSymbol = parseChordSymbol;
exports.getChordIntervals = getChordIntervals;
...
exports.SUPPORTED_CHORD_SUFFIXES = Object.keys(DEFINITIONS)
    .filter(Boolean)
...
function parseChordSymbol(symbol) {
    ...
    const root = (0, notes_js_1.normalizeNote)(`${letter?.toUpperCase()}${accidental}`);
    ...
    const bass = bassLetter
        ? (0, notes_js_1.normalizeNote)(`${bassLetter.toUpperCase()}${bassAccidental}`)
        : undefined;
    return {
...
function getChordIntervals(symbol) {
    ...
    const definition = Object.values(DEFINITIONS).find(item => item.quality === parsed.quality && item.suffix === parsed.suffix);
    if (!definition) {
        throw new Error(`Definição não encontrada para ${symbol}`);
    }
    return [...definition.intervals];
}
function getChordNotes(symbol) {
    const parsed = parseChordSymbol(symbol);
    const rootPitchClass = (0, notes_js_1.noteToPitchClass)(parsed.root);
    const intervals = getChordIntervals(symbol);
    const seen = new Set();
    for (const interval of intervals) {
        seen.add((0, notes_js_1.pitchClassToNote)(rootPitchClass + interval));
    }
    return [...seen];
}
