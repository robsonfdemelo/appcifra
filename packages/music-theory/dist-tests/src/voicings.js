...
exports.generateVoicings = generateVoicings;
exports.formatFretPattern = formatFretPattern;
const chords_js_1 = require("./chords.js");
const notes_js_1 = require("./notes.js");
const tunings_js_1 = require("./tunings.js");
const PREFERRED = {
    C: [
...
function noteAtFret(openString, fret) {
    return ((0, notes_js_1.noteToPitchClass)(openString) + fret) % 12;
}
function getAllowedFrets(openString, chordPitchClasses, maxFret) {
    const result = [-1];
    for (let fret = 0; fret <= maxFret; fret += 1)
        if (chordPitchClasses.has(noteAtFret(openString, fret)))
            result.push(fret);
    return result;
}
function cartesianProduct(values, index = 0, current = [], output = []) {
    ...
    for (const value of values[index] ?? []) {
        current.push(value);
        cartesianProduct(values, index + 1, current, output);
        current.pop();
    }
    return output;
}
function scoreVoicing(frets, rootPitchClass, tuning, fingers) {
...
function isValidVoicing(frets, chordPitchClasses, rootPitchClass, tuning, maxSpan, minStrings) {
    const sounding = frets.filter(fret => fret >= 0);
    if (sounding.length < minStrings)
        return false;
    const positiveFrets = frets.filter(fret => fret > 0);
    if (positiveFrets.length > 1 && Math.max(...positiveFrets) - Math.min(...positiveFrets) > maxSpan)
        return false;
    const soundedPitchClasses = new Set();
    let hasRoot = false;
    frets.forEach((fret, stringIndex) => { const open = tuning.strings[stringIndex]; if (fret < 0 || !open)
        return; const pc = noteAtFret(open, fret); soundedPitchClasses.add(pc); if (pc === rootPitchClass)
        hasRoot = true; });
    if (!hasRoot)
        return false;
    for (const pitchClass of chordPitchClasses)
        if (!soundedPitchClasses.has(pitchClass))
            return false;
    return true;
}
function preferredVoicings(symbol, tuning, rootPitchClass) {
    if (tuning.id !== tunings_js_1.GUITAR_STANDARD.id)
        return [];
    return (PREFERRED[symbol] ?? []).map(shape => scoreVoicing(shape.frets, rootPitchClass, tuning, shape.fingers)).filter((item) => !!item).map(item => ({ ...item, chord: symbol, difficulty: Math.max(1, Math.min(item.difficulty, 4)) }));
}
function generateVoicings(options) {
    const tuning = options.tuning ?? tunings_js_1.GUITAR_STANDARD;
    ...
    const parsed = (0, chords_js_1.parseChordSymbol)(options.chord);
    const chordPitchClasses = new Set((0, chords_js_1.getChordNotes)(options.chord).map(notes_js_1.noteToPitchClass));
    const rootPitchClass = (0, notes_js_1.noteToPitchClass)(parsed.root);
    ...
    for (const frets of combinations) {
        if (!isValidVoicing(frets, chordPitchClasses, rootPitchClass, tuning, maxSpan, minStrings))
            continue;
    ...
    return [...curated, ...generated.sort((a, b) => a.difficulty - b.difficulty || a.firstFret - b.firstFret || a.mutedStrings - b.mutedStrings || b.openStrings - a.openStrings)]
        .filter(item => { const key = item.frets.join(','); if (seen.has(key))
        return false; seen.add(key); return true; })
        .slice(0, limit);
}
function formatFretPattern(frets) {
